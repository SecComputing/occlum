...
run(){
  ...
  do
    ...
    ./bin/server --port=${port} --test_name="Server_"${test_name}&
    sleep 1
    server_pid=`ps -a | grep server | grep -v grep | sed 's/^\s*//' | sed 's/[[:space:]].*//'`
    # Launch the client
    ./bin/client --port=${port} --d=${dur} --w=${warmup} --r=${nr} --c=${nc} --req=${req_sz} --resp=${resp_sz} --rpc_type=${r_type}  --test_name="client_"${test_name}
    client_status=$(echo $?)
